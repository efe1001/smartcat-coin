...
function buildMockQuote(inputMint: string, outputMint: string, sol: number, slippageBps: number): QuoteResponse {
  ...
  return { inputMint, inAmount: String(Math.round(sol * 1e9)), outputMint, outAmount: String(out), priceImpactPct: "0.0012", routePlan: [], slippageBps, otherAmountThreshold: "0", swapMode: "ExactIn" };
}

const SLIPPAGE = [{ label: "0.5%", bps: 50 }, { label: "1%", bps: 100 }, { label: "2%", bps: 200 }];
    ...
    <div id="swap" className="grad-border rounded-2xl">
      <div className="glass rounded-2xl p-5">
        ...
        <div className="mb-4 flex items-center justify-between">
          <h3 className="font-bold text-white text-base">Swap</h3>
          <button
            ...
            <div className="flex gap-2">
              {SLIPPAGE.map(opt => (
                <button
                  ...
                  className={`rounded-lg px-3 py-1.5 text-xs font-semibold transition ${slippageBps === opt.bps ? "bg-orange-500 text-white" : "bg-white/5 text-gray-400 hover:text-white"}`}
                >
                  {opt.label}
                </button>
              ))}
        ...
        <div className="rounded-xl border border-white/[0.07] bg-white/[0.03] p-4 transition hover:border-white/[0.12]">
          ...
          <div className="flex items-center gap-3">
            ...
            <input
              ...
              className="swap-input flex-1 rounded-xl bg-transparent px-3 py-2 text-right text-xl font-bold text-white placeholder-gray-700 min-w-0"
            />
          </div>
        ...
        {quote && !loading && (
          <div className="mt-3 space-y-1.5 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 text-xs">
            ...
            <div className="flex justify-between text-gray-500">
              <span>Slippage</span>
              <span className="font-semibold text-gray-300">{slippageBps / 100}%</span>
            </div>
          </div>
        )}
        ...
        {/* Success */}
        {txState === "success" && (
          <div className="mt-3 rounded-xl border border-green-500/20 bg-green-500/10 px-4 py-3 text-xs text-green-400">
            ✓ Swap confirmed! &nbsp;
            <a href="https://solscan.io" target="_blank" rel="noopener noreferrer" className="underline hover:text-green-300">
              View on Solscan
            </a>
          </div>
        )}
